...
def plot_statistic(data_files, stat_type, output_format=".png"):
    ...
    data_dict = {
        ...
        "f_ng": {
            "col": None,
            "output_name": "All_Sims_fng_v_redshift",
        ...
        "sigma_var_mean": {
            "col": None,
            "output_name": "All_Sims_sigma_var_mean_v_redshift",
    ...
    for filename in data_files:
        ...
        if sim_name:
            ...
            L100stat = interpolate.interp1d(data[0], data[1], fill_value="extrapolate")
    ...
    for filename in data_files:
        ...
        redshifts = data[0]
        if (sim_name in ["RefL0025N0376"]) and (stat_type in ["sigma_ci", "sigma_var"]):
        ...
        if isinstance(data_dict[stat_type]["col"], int):
            ...
            if stat_type == "mean":
                print("TEST")
                stat = data[1]
                figg, axx = plt.subplots(ncols=1, nrows=2, constrained_layout=True)
                lcolor, lwidth, lstyle = plot_style_dict[sim_name]

                print(stat)
                axx[0].plot(redshifts, stat,
                    color=lcolor,
                    linewidth=lwidth,
                    linestyle=lstyle,
                ...
                axx[1].plot(redshifts, (stat - new_stat)/new_stat,
                    color=lcolor,
                    linewidth=lwidth,
                    linestyle=lstyle,
                    )
                ...
                lcolor, lwidth, lstyle = plot_style_dict[sim_name]
                p13 = acosmo.Planck13
                ax_twin = math_tools.cosmology.make_lookback_time_axis(axx[0], cosmo=p13, z_range=(redshifts[0], redshifts[-1]))
                ax_twin.set_xlabel("$\mathrm{Lookback\ Time\ [Gyr]}$")

                axx[0].legend()
                axx[1].set_xlabel("\\textrm{{Redshift}}")
                axx[0].set_ylabel(data_dict[stat_type]["ylabel"])
                print(f"./analysis_plots/shuffled/TEST_{data_dict[stat_type]['output_name']}{output_format}")
                plt.savefig(f"./analysis_plots/shuffled/TEST_{data_dict[stat_type]['output_name']}{output_format}")
...
if __name__ == "__main__":
    all_files = sorted(glob("./analysis_outputs/shuffled/ANALYSIS_*Re*mean_var_std_from_pdf*.txt"))
    all_files.reverse()
